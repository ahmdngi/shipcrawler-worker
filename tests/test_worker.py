        ...
        "results": [
            {
                ...
                "identity": {
                    ...
                    "gt": "22886", "dwt": "36903",
                    "year_built": "2017", "status": "In Service",
                },
                "position": {
                    "latitude": 59.5, "longitude": 24.5,
                    "destination": "TALLINN",
                    "speed": "0.0", "course": "0.0", "status": "Moored",
                },
...
class TestExecute:
    @patch("shipcrawler_worker.worker._ORCHESTRATE",
           Path("/nonexistent/path/orchestrate.py"))
    @pytest.mark.asyncio
    async def test_missing_mmsi(self, worker):
        """Task without mmsi should return failure."""
    ...
    async def test_successful_execution(
        ...
        with patch("shipcrawler_worker.worker._ORCHESTRATE", fake_orchestrate):
            with patch("shipcrawler_worker.worker.subprocess.run") as mock_run:
                mock_proc = MagicMock()
